...
public class UserDAO {
    ...
    public boolean checkDupicate(String userID) throws ClassNotFoundException, SQLException{
         boolean check = false;
...
public String getCurrentDateTime() throws ClassNotFoundException, SQLException {
    ...
    try {
        conn = DBUtils.getConnection();
        if (conn != null) {
            String GET_CURRENT_DATE_QUERY = "SELECT CURRENT_TIMESTAMP";
            ptm = conn.prepareStatement(GET_CURRENT_DATE_QUERY);
            rs = ptm.executeQuery();
            if (rs.next()) {
                Timestamp currentDateTime = rs.getTimestamp(1);
                // Format the date to DD-MM-YYYY HH:mm
                SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy HH:mm");
                formattedDateTime = sdf.format(currentDateTime);
            }
        }
    } catch (Exception e) {
        e.printStackTrace();
...
public UserDTO top1Server() throws SQLException{
    UserDTO user = null;
    Connection conn = null;
    PreparedStatement ptm = null;
    ResultSet rs = null;
